...
extern string IndExportFileName = "LastData.csv";
extern int  trainSize = 25;
...
//---
  ...
  // ArraySetAsSeries(WilliamsRArr, true);
   ...
   int copied = CopyRates(Symbol(), Period(), 1, trainSize, srcArr);
  ...
  //int hWilliamsR = iVolumes(Symbol(), Period(), VOLUME_TICK);
   ...
   CopyBuffer(hMA, 0, 1, trainSize, MAArr);
   CopyBuffer(hRSI, 0, 1, trainSize, RSIArr);
   CopyBuffer(hVolumes, 0, 1, trainSize, VolumesArr);
